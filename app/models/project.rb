    ...
    @unachieve_trophies.each do |unach_trophy|
      ...
      # 一度獲得すれば、↑の「獲得済み」を飛ばすで自動的に対象外になる。
        ...
        when '『ゴートの叫び』' then
          ...
          if self.achieve_trophy.exists?(:trophy_id => t_id) && self.commit_num == 0
            add_ach_trophy_id = Trophy.find_by(name: '『ゴートの叫び』').id
            puts("        → トロフィー『ゴートの叫び』解放")
          end
        when '『石の上にもヤギ』' then
          if self.sprint_continue_record > 4  # 連続記録が5を超えたとき
            add_ach_trophy_id = Trophy.find_by(name: '『石の上にもヤギ』').id
            puts("        → トロフィー『石の上にもヤギ』解放")
          end
      end
